    ...
    $.fn.Tabledit = function(options) {
            ...
            columns: {
                    ...
                    for (var i = 0; i < settings.columns.editable.length; i++) {
                        ...
                        $td.each(function() {
                            ...
                            if (typeof settings.columns.editable[i][2] !== 'undefined') {
                                ...
                                var input = '<input class="tabledit-input ' + settings.inputClass + '" type="text" name="' + settings.columns.editable[i][1] + '" value="' + $(this).text() + '" style="display: none;" disabled required>';
                            }
                ...
                toolbar: function() {
                    if (settings.editButton || settings.deleteButton) {
                        ...
                        // Create edit button.
                        if (settings.editButton) {
                            editButton = '<button type="button" class="tabledit-edit-button ' + settings.buttons.edit.class + '" style="float: none;">' + settings.buttons.edit.html + '<i class="fas fa-edit"></i></button>';
                        }

                        // Create delete button.
                        if (settings.deleteButton) {
                            deleteButton = '<button type="button" class="tabledit-delete-button ' + settings.buttons.delete.class + '" style="float: none;">' + settings.buttons.delete.html + '<i class="fas fa-trash-alt"></i></button>';
                            ConfirmarButton = '<button type="button" class="tabledit-Confirmar-button ' + settings.buttons.Confirmar.class + '" style="display: none; float: none;">' + settings.buttons.Confirmar.html + '</button>';
                        }

                        // Create Guardar button.
                        if (settings.editButton && settings.GuardarButton) {
                            GuardarButton = '<button type="button" class="tabledit-Guardar-button ' + settings.buttons.Guardar.class + '" style="display: none; float: none;">' + settings.buttons.Guardar.html +'</button>';
                        }
        ...
        var Mode = {
            view: function(td) {
                ...
                if (settings.editButton) {
                    $tr.find('button.tabledit-Guardar-button').hide();
            ...
            edit: function(td) {
                ...
                if (settings.editButton) {
                    ...
                    $tr.find('button.tabledit-Guardar-button').show();
                }
            }
        };
        ...
        var Delete = {
            reset: function(td) {
                ...
                $table.find('.tabledit-Confirmar-button').hide();
            ...
            Confirmar: function(td) {
                ...
                $(td).find('.tabledit-Confirmar-button').show();
            },
            restore: function(td) {
                // Enable identifier hidden input.
                $(td).parent('tr').find('input.tabledit-identifier').attr('disabled', false);
        ...
        if (settings.deleteButton) {
            ...
            /**
             * Delete one row (Confirmar).
             *
             * @param {object} event
             */
            $table.on('click', 'button.tabledit-Confirmar-button', function(event) {
                if (event.handled !== true) {
                    event.preventDefault();
        ...
        if (settings.editButton) {
             ...
             * @param {object} event
             */
            $table.on('click', 'button.tabledit-Guardar-button', function(event) {
                if (event.handled !== true) {
                    event.preventDefault();
        ...
        $(document).on('keyup', function(event) {
            // Get input element with focus or Confirmaration button.
            var $input = $table.find('.tabledit-input:visible');
            var $button = $table.find('.tabledit-Confirmar-button');
